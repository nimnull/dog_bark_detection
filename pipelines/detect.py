...
def determine_time_regions(
    centroids, freq_bins, dbs, sampling, freq_range, db_range, duration
):
    """
    Here we determine the number of regions where bark occurs.
    """
    freq_lower, freq_higher = freq_range

    freq_idxs = np.where((freq_bins >= freq_lower) & (freq_bins <= freq_higher))
    ...
    for idx, grid in enumerate(duration):
        cur_regions = find_regions(
            centroids, dbs_averaged, sampling, freq_range, db_range, grid
        )
        if idx >= center_idx:
...
def occupancy_in_range(array, any_range):
    """
    Calculates the proportion of values in the given range.
    """
    range_lower, range_higher = any_range
    idxs = np.where((array >= range_lower) & (array <= range_higher))[0]
